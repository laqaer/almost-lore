...
| Cloudflare account `Laqaer Products` | Workers free tier. Subdomain `laqaer-products`. Zone for almostlore.com is not on this account. KV `almost-lore-ops` / `3afda249787643dd9e7483b316c03619`. | Worker `https://almost-lore-ops.laqaer-products.workers.dev`. Do not deploy over the script name `almost-lore`; that name is reserved for the static-export migration. |
| Stripe | MCP authentication was attempted on 2026-09-26 and timed out. No key is on the worker. | Required before the $9 button can appear. |
